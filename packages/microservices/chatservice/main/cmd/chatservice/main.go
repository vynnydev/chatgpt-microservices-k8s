...
import (
	...
	"github.com/vynnydev/chatgpt-microservices-k8s/packages/microservices/chatservice/infra/grpc/server"
	"github.com/vynnydev/chatgpt-microservices-k8s/packages/microservices/chatservice/infra/repository"
...
func main() {
	...
	usecases := chatcompletion.NewChatCompletionusecases(repo, client)

	streamChannel := make(chan chatcompletionstream.ChatCompletionOutputDTO)
	usecasestream := chatcompletionstream.NewChatCompletionusecases(repo, client, streamChannel)
	...
	webserverChatHandler := web.NewWebChatGPTHandler(*usecases, chatConfig, configs.AuthToken)
