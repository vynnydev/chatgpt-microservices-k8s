...
import (
	...
	"github.com/vynnydev/chatgpt-microservices-k8s/packages/microservices/chatservice/domain/application/repository"
...
type ChatCompletionusecases struct {
	ChatRepository repository.ChatRepository
	OpenAIClient   *openai.Client
	Stream         chan ChatCompletionOutputDTO
}

func NewChatCompletionusecases(chatRepository repository.ChatRepository, openAIClient *openai.Client, stream chan ChatCompletionOutputDTO) *ChatCompletionusecases {
	return &ChatCompletionusecases{
...
func (uc *ChatCompletionusecases) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionOutputDTO, error) {
	chat, err := uc.ChatRepository.FindChatByID(ctx, input.ChatID)
	if err != nil {
		if err.Error() == "chat not found" {
			...
			err = uc.ChatRepository.CreateChat(ctx, chat)
			if err != nil {
	...
	err = uc.ChatRepository.SaveChat(ctx, chat)
	if err != nil {
		return nil, err
	}
